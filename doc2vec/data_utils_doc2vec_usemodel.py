...
class Corpus(object):
    ...
    def get_data(self, node_label_file_1,edge_file_1,node_label_file_2,edge_file_2,training_file,func_matching_file,subject_dir,pretrained_subject, with_gt):
        ...
        all_func_emb2 = []
        all_func2 = []
        f = open(subject_dir + "/funcname1.txt", "w")
        for func1 in src_func_node_list.keys():
            node_ids1 = src_func_node_list[func1]
            embs1 = embedding_1[node_ids1]
            func_emb1 = torch.mean(embs1, dim=0)
            all_func_emb1.append(func_emb1)
            f.write(func1 + "\n")
        f.close()
        
        f = open(subject_dir + "/funcname2.txt", "w")
        for func2 in des_func_node_list.keys():
            node_ids2 = des_func_node_list[func2]
            embs2 = embedding_2[node_ids2]
            func_emb2 = torch.mean(embs2, dim=0)
            all_func_emb2.append(func_emb2)
            f.write(func2 + "\n")
        f.close()
        f = open(subject_dir + "/funcemb1.pkl", "wb")
        pickle.dump(torch.stack(all_func_emb1), f, protocol = 4)
        f.close()
        f = open(subject_dir + "/funcemb2.pkl", "wb")
        pickle.dump(torch.stack(all_func_emb2), f, protocol = 4)
        f.close()
